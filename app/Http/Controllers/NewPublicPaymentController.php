...
use App\Http\Requests\PublicPayment\PersonalDataRequest;
...
class NewPublicPaymentController extends Controller
{
    function show(Payment $payment, Request $request)
    {
        ...
        return view('public-multiple.payment')
        ->with('payment', $payment)
        ->with('card_brands', $availableBrands);
    }

    function personal(Payment $payment, PersonalDataRequest $request)
    {
        ...
        // TODO: prepare for custom inputs
        ...
        return redirect('/pay/' . $payment->id . '?page=address')->with('customer', $customer);
    }

    function address(Payment $payment, PersonalDataRequest $request)
    {
        $data = $request->validated();
